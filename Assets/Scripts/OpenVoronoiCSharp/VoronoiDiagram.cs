...
namespace OpenVoronoiCSharp
{
    class VoronoiDiagram
    {
        ...
        protected List<Face> incident_faces = new List<Face>(); ///< temporary variable for ::INCIDENT faces, will be reset to ::NONINCIDENT after a site has been inserted
        protected HashSet<Vertex> modified_vertices = new HashSet<Vertex>(); ///< temporary variable for in-vertices, outv-vertices that need to be reset after a site has been inserted
        protected List<Vertex> v0 = new List<Vertex>(); ///< IN-vertices, i.e. to-be-deleted
        protected bool debug; ///< turn debug output on/off
        protected bool silent; ///< no warnings emitted when silent==true
        ...
        public Vertex insert_point_site(Point p, int step)
        {
            ...
            {
                string s = String.Format("openvoronoi error. All points must lie within unit-circle. You are trying to Add p= %s with p.norm()= %f\n", p, p.norm());
                Debug.Assert(false, s);
            }
            Debug.Assert(p.norm() < far_radius, " p.norm() < far_radius ");
            ...
            foreach (Face f in incident_faces)
            {
            ...
            Debug.Assert(vd_checker.face_ok(newface), " vd_checker.face_ok( newface ) ");
            Debug.Assert(vd_checker.is_valid(), " vd_checker.is_valid() ");

            return new_vert;
        }

        public bool insert_line_site(Vertex v1, Vertex v2)
        {
            // default step should make algorithm run until the end!
            return insert_line_site(v1, v2, 99);
        }

        public bool insert_line_site(Vertex start, Vertex end, int step)
        {
            ...
            Debug.Assert(vd_checker.face_ok(start.face), " vd_checker.face_ok( start.face ) ");
            ...
            Debug.Assert(vd_checker.face_ok(end.face), " vd_checker.face_ok( end.face ) ");
            ...
            foreach (Face f in incident_faces)
            {
                ...
                {// end-point faces already dealt with in add_separator()
            ...
            Debug.Assert(vd_checker.face_ok(pos_face), " vd_checker.face_ok( pos_face ) ");

            repair_face(neg_face, new Pair<Vertex, Vertex>(seg_start, seg_end),
                                   new Pair<Face, Face>(start_to_null, end_to_null),
                                   new Pair<Face, Face>(start_null_face, end_null_face));
            Debug.Assert(vd_checker.face_ok(neg_face), " vd_checker.face_ok( neg_face ) ");
            ...
            Debug.Assert(vd_checker.face_ok(start_null_face), " vd_checker.face_ok( start_null_face ) ");
            Debug.Assert(vd_checker.face_ok(end_null_face), " vd_checker.face_ok( end_null_face ) ");
            Debug.Assert(vd_checker.face_ok(pos_face), " vd_checker.face_ok( pos_face ) ");
            Debug.Assert(vd_checker.face_ok(neg_face), " vd_checker.face_ok( neg_face ) ");
            Debug.Assert(vd_checker.is_valid(), " vd_checker.is_valid() ");
        ...
        //    {
        //        return -Double.(Math.abs(lhs.getSecond()), Math.abs(rhs.getSecond()));
        //    }
        //}
        ...
        protected void initialize()
        {
            ...
            g.set_next_cycle(new List<Edge> { e1_1, e1_2, e2, e3_1, e3_2 }, f1, 1);
            ...
            g.set_next_cycle(new List<Edge> { e4_1, e4_2, e5, e6_1, e6_2 }, f2, 1);
            ...
            g.set_next_cycle(new List<Edge> { e7_1, e7_2, e8, e9_1, e9_2 }, f3, 1);
        ...
        protected Vertex find_seed_vertex(Face f, Site site)
        {
            ...
            Debug.Assert(minPred < 0, " minPred < 0 ");
        ...
        {
            Debug.Assert(v0.Count != 0, " v0.Count!=0 ");
            List<Edge> output = new List<Edge>(); // new vertices generated on these edges
            foreach (Vertex v in v0)
            {
                Debug.Assert(v.status == VertexStatus.IN, " v.status == VertexStatus.IN ");
            ...
            Debug.Assert(output.Count != 0, " !output.isEmpty() ");
            return output;
        }
        ...
        public EdgeData find_edge_data(Face f, List<Vertex> startverts, Pair<Vertex, Vertex> segment)
        {
            ...
            bool found = false;
            do
            { // find OUT-NEW-IN vertices in this loop
                ...
                bool previous_not_endpoint = (!previous_vertex.Equals(segment.getFirst()) && !previous_vertex.Equals(segment.getSecond()));
                bool next_is_endpoint = (next_vertex.Equals(segment.getFirst()) || next_vertex.Equals(segment.getSecond()));
                   ...
                   )
                {
                    ...
                    bool v_in_startverts = startverts.Contains(current_vertex);
                    if (!v_in_startverts)
                    {
            ...
            Debug.Assert(found, "found");

            // now search for v2
            start_edge = current_edge; // note that this search starts where we ended in the loop above!
            found = false;
            do
            { // find IN-NEW-OUT vertices in this loop
                Vertex current_vertex = current_edge.target;
                if (current_vertex.status == VertexStatus.NEW && current_vertex.type != VertexType.SEPPOINT)
                {
                    if (!current_vertex.Equals(ed.v1))
                    { // -IN-NEW(v2)
        ...
        {
            Debug.Assert(vd_checker.face_ok(f), " vd_checker.face_ok(f) ");
            List<Edge> outv = new List<Edge>();
            Edge current_edge = f.edge;
            Edge start_edge = current_edge;
            //int count=0;
            do
            { // FIND ALL! not just one.
                Vertex trg = current_edge.target;
                Vertex src = current_edge.source;
                bool src_is_right = src.position.is_right(pt1, pt2);
                bool trg_is_right = trg.position.is_right(pt1, pt2);
                if (src.type == VertexType.NORMAL || src.type == VertexType.APEX || src.type == VertexType.SPLIT)
                { //? check edge-type instead?
                    if (src_is_right != trg_is_right)
                    {
                        outv.Add(current_edge);
                        Debug.Assert(vd_checker.check_edge(current_edge), "vd_checker.check_edge(current_edge)");
                    }
                }
                current_edge = current_edge.next;
        ...
        {
            foreach (Vertex q in g.face_vertices(f))
            {
        ...
        protected void augment_vertex_set(Site site)
        {
            while (vertexQueue.Count != 0)
            {
                ...
                if (h < 0.0)
                { // try to mark IN if h<0 and passes (C4) and (C5) tests and in_region(). otherwise mark OUT
            ...
            Debug.Assert(vertexQueue.Count == 0, " vertexQueue.isEmpty() ");
            Debug.Assert(vd_checker.all_in(v0), " vd_checker.all_in(v0) ");
        ...
        protected bool predicate_c4(Vertex v)
        {
            int in_count = 0;
            foreach (Edge e in v.out_edges)
            {
        ...
        protected bool predicate_c5(Vertex v)
        {
            if (v.type == VertexType.APEX || v.type == VertexType.SPLIT) { return true; } // ?
            List<Face> adjacent_incident_faces = new List<Face>();
            ...
            Debug.Assert(adjacent_incident_faces.Count != 0, " !adjacent_incident_faces.isEmpty() ");

            foreach (Face f in adjacent_incident_faces)
            {
                ...
                do
                {
                    ...
                    if (!w.Equals(v) && w.status == VertexStatus.IN && g.has_edge(w, v))
                    { // v should be adjacent to an IN vertex on the face
                        face_ok = true;
                    }
                    else if (!w.Equals(v) && (w.type == VertexType.ENDPOINT || w.type == VertexType.APEX || w.type == VertexType.SPLIT))
                    {// if we are next to an ENDPOINT, then ok(?)
                        face_ok = true;
                    }
        ...
        protected void mark_adjacent_faces(Vertex v, Site site)
        {
            ...
            Debug.Assert(
                (v.type == VertexType.APEX && new_adjacent_faces.Count == 2) ||
                (v.type == VertexType.SPLIT && new_adjacent_faces.Count == 2) ||
                new_adjacent_faces.Count == 3
            );

            foreach (Face adj_face in new_adjacent_faces)
            {
        ...
        protected void add_edges(Face newface, Face f, Face newface2, Pair<Vertex, Vertex> segment)
        {
            ...
            Debug.Assert(new_count > 0, " new_count > 0 ");
            Debug.Assert((new_count % 2) == 0, " (new_count % 2) == 0 ");
            int new_pairs = new_count / 2; // we Add one NEW-NEW edge for each pair found
            List<Vertex> startverts = new List<Vertex>(); // this holds ed.v1 vertices for edges already added
        ...
        protected void add_edge(EdgeData ed, Face newface, Face newface2)
        {
            ...
            Face new_face;
            if (new_source.k3 == 1)
            { // find outv if newface or newface2 should be used
            ...
            Debug.Assert(new_target.k3 == new_source.k3, " new_target.k3 == new_source.k3 ");
            ...
            { // LL
              ...
              // this is essentially an in-region test
                ...
                {
                    Debug.Assert(!new_source.position.is_right(f_site.start(), f_site.end()), " !new_source.position.is_right( f_site.start(), f_site.end() ) ");
                    Debug.Assert(!new_target.position.is_right(f_site.start(), f_site.end()), " !new_target.position.is_right( f_site.start(), f_site.end() ) ");
                    Debug.Assert(!new_source.position.is_right(new_site.start(), new_site.end()), " !new_source.position.is_right( new_site.start(), new_site.end() ) ");
                    Debug.Assert(!new_target.position.is_right(new_site.start(), new_site.end()), " !new_target.position.is_right( new_site.start(), new_site.end() ) ");
                }
            }
            else if (f_site.isLine() && new_site.isArc())
            { // LA
            ...
            { // unhandled case!
                throw new Exception(" add_edge() WARNING: no code to deremine src_sign and trg_sign!");
            }
            ...
            {  // Add a single src-trg edge
                ...
                e_new.next = new_next;
                Debug.Assert(new_next.k == new_previous.k, " new_next.k == new_previous.k ");
                ...
                Debug.Assert(vd_checker.check_edge(e_new) && vd_checker.check_edge(e_twin), " vd_checker.check_edge(e_new) && vd_checker.check_edge(e_twin) ");
            }
            else
            {
                ...
                Debug.Assert(new_previous.face == f, " new_previous.face == f ");
                Debug.Assert(new_next.face == new_previous.face, " new_next.face == new_previous.face ");
                Debug.Assert(new_next.k == new_previous.k, " new_next.k == new_previous.k ");
                ...
                Debug.Assert(twin_previous.k == twin_next.k, " twin_previous.k == twin_next.k ");
                Debug.Assert(twin_previous.face == twin_next.face, " twin_previous.face == twin_next.face ");
                ...
                Debug.Assert(vd_checker.check_edge(e1) && vd_checker.check_edge(e1_tw), " vd_checker.check_edge(e1) && vd_checker.check_edge(e1_tw) ");
                Debug.Assert(vd_checker.check_edge(e2) && vd_checker.check_edge(e2_tw), " vd_checker.check_edge(e2) && vd_checker.check_edge(e2_tw) ");
        ...
        protected void add_separator(Face f, Face null_face, SeparatorTarget target, Vertex sep_endp, Site s1, Site s2)
        {
            ...
            Debug.Assert((sep_endp.k3 == 1) || (sep_endp.k3 == -1), " (sep_endp.k3==1) || (sep_endp.k3==-1) ");
            sep_endp.zero_dist();
            ...
            Debug.Assert(endp_next != null, " endp_next != null ");
            Debug.Assert(endp_prev != null, " endp_prev != null ");
            ...
            bool out_new_in = target.out_new_in;
            Debug.Assert((v_target.k3 == 1) || (v_target.k3 == -1), " (v_target.k3==1) || (v_target.k3==-1) ");
            Debug.Assert(sep_endp.k3 == v_target.k3, " sep_endp.k3 == v_target.k3 ");
            // can't Debug.Assert about in_region - numerical error is always present
            // Debug.Assert( s1.in_region(v_target.position ) , " s1.in_region(v_target.position ) ");
            // Debug.Assert( s2.in_region(v_target.position ) , " s2.in_region(v_target.position ) ");
            ...
            if (out_new_in)
            {
                ...
                if (e2.k == -1)
                { // choose either s1 or s2 as the site
                    e2.face = s2.face;
                    s2.face.edge = e2;
            ...
            Debug.Assert(vd_checker.check_edge(e2), " vd_checker.check_edge(e2) ");
            Debug.Assert(vd_checker.check_edge(e2_tw), " vd_checker.check_edge(e2_tw) ");
        ...
        {
            Debug.Assert(!s.isPoint(), "!s.isPoint()");
            ...
            if (fs.isPoint() && s.isLine() && s.in_region(fs.position()))
            {
                ...
                Debug.Assert((pt1.sub(pt2)).norm() > 0, " (pt1.sub(pt2)).norm() > 0 ");
                ...
                foreach (Edge split_edge in split_edges)
                {
                    ...
                    Debug.Assert(vd_checker.check_edge(split_edge), " vd_checker.check_edge(split_edge) ");
        ...
        protected FiveTuple1 find_null_face(Vertex start, Vertex other, Point left, Point dir, Site new_site)
        {
            ...
            { // there is an existing null face
                ...
                do
                {
                    bool face_incident = (current2.twin.face.status == FaceStatus.INCIDENT);
                    if (face_incident)
                    { // pick any incident face!
                        insert_edge = current2;
                        found = true;
                    }
                    current2 = current2.next;
                } while (!current2.Equals(start_edge2) && !found); // FIXME end early with !found
                Debug.Assert(insert_edge != null, " insert_edge != null");
                Debug.Assert(found, " found ");
            ...
            { // no existing null-face
              ...
              //
                ...
                g.set_next_cycle(new List<Edge> { e1, e2, e3 }, start_null_face, 1);
                ...
                g.set_next_cycle(new List<Edge> { e3_tw, e2_tw, e1_tw }, start.face, 1);
        ...
        protected SeparatorTarget find_separator_target(Face f, Vertex endp)
        {
            ...
            do
            {
                ...
                Vertex current_vertex = current_edge.target;
                Vertex next_vertex = next_edge.target;
                bool out_new_in = (((previous_vertex.status == VertexStatus.OUT) || (previous_vertex.status == VertexStatus.UNDECIDED)) &&
                                     current_vertex.status == VertexStatus.NEW &&
                                     next_vertex.status == VertexStatus.IN);
                bool in_new_out = (previous_vertex.status == VertexStatus.IN &&
                                    current_vertex.status == VertexStatus.NEW &&
        ...
        protected Pair<Vertex, Face> process_null_edge(Point dir, Edge next_edge, bool k3, bool next_prev)
        {
            Debug.Assert(next_edge.type == EdgeType.NULLEDGE, " next_edge.type == EdgeType.NULLEDGE ");
            ...
            Debug.Assert((next_prev ? src : trg).type == VertexType.ENDPOINT, " (next_prev ? src : trg).type == VertexType.ENDPOINT ");
            ...
            if (adj.type == VertexType.ENDPOINT)
            { // target is endpoint
            ...
            else
            {
                ...
                Debug.Assert(adj.type != VertexType.ENDPOINT, " adj.type != VertexType.ENDPOINT ");
                double mid = 0;
                ...
                Debug.Assert(adj_out != null, "adj_out != null");
                                     ...
                                     Pair<Face, Face> null_face)
        {
            ...
            do
            {
                ...
                foreach (Edge e in current_target.out_edges)
                {
                    ...
                    { // these are the possible vertices we want to go to
                        ...
                        if (e.face == f)
                        {
                            ...
                            Debug.Assert(current_edge.k == e.k, " current_edge.k == e.k ");
                            Debug.Assert(vd_checker.current_face_equals_next_face(current_edge), " vd_checker.current_face_equals_next_face( current_edge ) ");
                        }
                    }
                }
        ...
        protected void remove_split_vertex(Face f)
        {
            ...
            {
                Debug.Assert(v.type == VertexType.SPLIT, "v.type == VertexType.SPLIT");
        ...
        {
            foreach (Vertex v in modified_vertices)
            {
                v.reset_status();
            }
    ...
    class EdgeData
    {
        public Edge v1_prv; ///< edge prior to v1
        public Vertex v1;   ///< NEW edge source
        public Edge v1_nxt; ///< edge following v1
        public Edge v2_prv; ///< edge prior to v2
        public Vertex v2;   ///< NEW edge target
    ...
    class SeparatorTarget
    {
        ...
        public Edge v_next;
        public bool out_new_in;
        public SeparatorTarget(Edge v_previous, Vertex v_target, Edge v_next, bool out_new_in)
        {
