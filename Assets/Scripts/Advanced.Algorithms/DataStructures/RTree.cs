...
namespace Advanced.Algorithms.DataStructures
{
    ...
    /// </summary>
    public class RTree<T> : IEnumerable<Polygon> where T : SpatialData
    {
        ...
        /// </summary>
        public List<T> RangeSearch(Rectangle searchRectangle)
        {
            return rangeSearch(Root, searchRectangle, new List<T>());
        }
        /// <summary>
        ///     Returns a list of polygons that's contained within given search rectangle.
        /// </summary>
        private List<T> rangeSearch(RTreeNode current, Rectangle searchRectangle, List<T> result)
        {
            if (current.IsLeaf)
            {
                foreach (var node in current.Children.Take(current.KeyCount))
                {
                    ...
                    {
                        result.Add(node.MBRectangle.Polygon as T);
                    }
                }
            }
